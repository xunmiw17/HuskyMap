...
public class GraphTestDriver {
    ...
    private void listChildren(String graphName, String parentName) {
        ...
        for (int i = 0; i < sortedList.size() - 1; i++) {
            Graph.DirectedLabeledEdge<String, String> edge = sortedList.get(i);
            output.print(" " + edge.getChild() + "(" + edge.getLabel() + ") ");
        }
        if (sortedList.size() > 0) {
            output.print(" " + sortedList.get(sortedList.size() - 1).getChild() + "(" + sortedList.get(sortedList.size() - 1).getLabel() + ")");
        }
        output.println();
    }
